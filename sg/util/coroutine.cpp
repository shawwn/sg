...
constexpr bool is_seq_of_maybe_v() { return true; }
template<typename T>
constexpr bool is_seq_of_maybe_v() { return false; }
...
auto len(T&& seq) { return std::size(seq); }
...
requires requires(T t) { std::is_same_v<decltype(seq_value_type_v(t)), V<decltype(seq_value_type_v(t))>>; }
auto maybes2(T seq) {
  return std::move(seq);
}
...
auto range(auto stop) -> Generator<decltype(stop)> {
  yield_from(range(static_cast<decltype(stop)>(0), stop, static_cast<decltype(stop)>(1)));
}
...
auto enumerate(auto&& l, int start = 0) -> Generator<decltype(std::make_pair(start, *std::begin(l)))> {
  for (auto v : l)
...
//  return append_(append_(make(l), l), x);
//}
auto append(auto&& l, auto&& ... ys) {
  return append_(make(l), l, ys...);
}
#endif
...
//auto list(auto&& l, auto as = std::vector<decltype(value_type_t(l))>()) -> decltype(auto) {
//  decltype(as) out;
//  append_(out, l);
//  return out;
//}
template<typename T>
constexpr bool can_print() { return false; }
...
//    }
//    std::cout << std::endl;
    prc("foo", 42, l, std::map<std::string, int>{{"foo", 42}}, sg::fibonacci_sequence<decltype(3)>, fibonacci_sequence(3), ([=]() -> Generator<float> {
        yield 42.9;
...
//          yield it;
  ...
  {
    for (auto n : range(5)) pr(n);
    prn();
    for (auto n : reversed(range(5))) pr(n);
    prn();
    auto l = std::vector<int>{1,2,3};
    for (auto n : reversed(l)) pr(n);
    prn();
    prn(len(l));
    prn(len(reversed(l)));
    prn(len("foobar"));
