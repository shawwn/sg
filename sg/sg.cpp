...
offsetof(structure, field)); #define PRN_STRUCT_OFFSETS(field)
PRN_STRUCT_OFFSETS_(struct a, field)
...
#define NUM_ARGS(...)                                                          \
  EXPAND(_GET_NTH_ARG(__VA_ARGS__, 9, 8, 7, 6, 5, 4, 3, 2, 1, 0))
...
auto literal(const char* x) { display(std::quoted(x)); }
auto literal(const std::string& x) { display(std::quoted(x)); }
auto literal(const std::string_view& x) { display(std::quoted(x)); }
auto literal(auto x) { display(x); }
...
void display(const char* x) { std::cout << x; }
void display(const std::string_view& x) { std::cout << x; }
void display(const std::string& x) { display(x.c_str()); }
void display(decltype(nil)) { display("nil"); }
template <typename T>
...
// template<StringLiteral open = "", StringLiteral sep = " ", StringLiteral
...
auto tuple(auto... xs) { return std::make_tuple(xs...); }

auto list(auto x) { return std::vector<decltype(x)>{x}; }

auto list(auto x, auto... args) { return std::vector<decltype(x)>{x, args...}; }

auto append(auto l, auto... ls) { return update(decltype(l){}, l, ls...); }
...
void sg::test_sg() {
  ...
  }
  {
    std::unordered_map<int, decltype(list("foo"))> u = {
        {1, list("#FF0000")}, {2, list("#00FF00")}, {3, list("#0000FF")}};
    prn(u);
  }
  prn(nil);
  //  auto m = maybe<int>(1);
  ...
  //  print("std::is_same_v<maybe<decltype(not_maybe)::value_type>,
  //  decltype(not_maybe)> = ",
  //        std::is_same_v<maybe<decltype(not_maybe)::value_type>,
  //        decltype(not_maybe)>);
  display("std::is_same_v<maybe<as_value_type_v<decltype(not_maybe)>>, "
          "decltype(not_maybe)> = ");
  prn(std::is_same_v<maybe<as_value_type_v<decltype(not_maybe)>>,
      decltype(not_maybe)>);
  display(
      "std::is_same_v<maybe<as_value_type_v<decltype(m)>>, decltype(m)> = ");
  prn(std::is_same_v<maybe<as_value_type_v<decltype(m)>>, decltype(m)>);
  //  prn(std::is_same_v<maybe<decltype(value_type_f<decltype(m)>())>,
  //  decltype(m)>); prn(sg::type_name(value_type_f<decltype(m)>()));
